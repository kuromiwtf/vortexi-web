...
def clear_all_auth_tokens():
    auth_token_keys = redis_controller.keys("authtoken_*")
    
    if not auth_token_keys:
        print("No auth tokens found")
        return 0
    
    deleted_count = redis_controller.delete(*auth_token_keys)
    
    print(f"Successfully cleared {deleted_count} auth tokens")
    return deleted_count

if config.USE_LOCAL_STORAGE:
    if not os.path.exists(config.R2_DOWNLOAD_CACHE_DIR):
        os.makedirs(config.R2_DOWNLOAD_CACHE_DIR)

    @AssetRoute.route("/cdn_local/<path:filehash>", methods=["GET"])
    def LocalCDN(filehash):
        if not os.path.exists(config.R2_DOWNLOAD_CACHE_DIR + "/" + filehash):
          return abort(404)

        if not bool(re.compile(r"^[a-fA-F0-9]{128}$").match(filehash)):
          return abort(404)

        with open(config.R2_DOWNLOAD_CACHE_DIR + "/" + filehash, "rb") as f:
            FileContents = f.read()
...
def AvatarFetch():
    ...
    if UserObj.id == 18:
        ...
        "accessoryVersionIds": AvatarAssetsList,
        "equippedGearVersionIds": ["1082"],
        "backpackGearVersionIds": ["1082"],
        "assetAndAssetTypeIds": assetAndAssetTypeIds,
        "bodyColors": {
            "HeadColor": PlayerAvatar.head_color_id,
            "LeftArmColor": PlayerAvatar.left_arm_color_id,
...
def GetOriginalAssetInfo(assetId, throwException = False, attempt = 0):
    ...
    try:
        if redis_controller.get(f"FetchAssetInfo_v2:EconomyAPI:{str(assetId)}:Blocked") is not None:
            if throwException:
                logging.warning("The asset is blocked from being fetched id: " + str(assetId))
        ...
        if config.ASSETMIGRATOR_USE_PROXIES:
            ...
            if assignedProxy is not None:
                """
                zer0 sağolsun roproxy diye bisi var oldugunu soyledi
                ...
                """
                logging.warning("Using proxy: " + assignedProxy)
                try:
                    assetInfoReq = requests.get(
                        f"https://economy.roblox.com/v2/assets/{str(assetId)}/details",
                        proxies = {
                    ...
                    logging.error("Failed to fetch asset info with proxy: " + assignedProxy + " error: " + str(e))
                    assetInfoReq = requests.get(
                        f"https://economy.roblox.com/v2/assets/{str(assetId)}/details"
                    )
                if "Too many requests" in assetInfoReq.text:
                    # try with roblox api
                    logging.warning("The ratelimit has been reached (roblox), trying with ROPROXY api")
                    assetInfoReq = requests.get(
                        f"https://economy.roproxy.com/v2/assets/{str(assetId)}/details",
                        proxies = {
                            "http": assignedProxy,
                            "https": assignedProxy
                        }
        ...
        if assetInfoReq.status_code != 200:
            if assetInfoReq.status_code == 429:
                if attempt < 2:
                    ...
                    if throwException:
                        logging.warning("The ratelimit has been reached status code: " + str(assetInfoReq.status_code) + " id: " + str(assetId) + " text: " + assetInfoReq.text + " url: "+ assetInfoReq.url)
...
def getAudioData_V2( assetId : int, placeId : int = None ) -> bytes:
    ...
    try:
        AssetFetchReq : requests.Response = RequestSession.get(
            url = f"https://assetdelivery.ttblox.mom/v1/asset/?id={str(assetId)}"
        )
    except Exception as e:
...
def getAudioData( assetId : int, placeid : int = None ) -> bytes:
    ...
    try:
        AssetFetchReq : requests.Response = RequestSession.post(
            f"https://assetdelivery.roproxy.com/v2/assets/batch",
...
def migrateAsset( assetid : int, forceMigration : bool = False, allowedTypes = [1,3,4,5,10,13,24,39,40,50,51,52,53,54,55,56], creatorId : int = 49, keepRobloxId : bool = True, migrateInfo : bool = True, attempt : int = 0, throwException : bool = False, allowBackgroundMigration : bool = False, assetVersionId : int = None, bypassCooldown : bool = False, attemptSoundWithPlaceId : int = -1 ) -> Asset:
    ...
    try:
        ...
        if throwException:
            logging.error(f"failed to migrate {assetid} is not found")
            raise AssetNotFoundException("The asset has not been found")
        return None
    except AssetNotAllowedException:
        if throwException:
            logging.error(f"failed to migrate {assetid} is not allowed")
            raise AssetNotAllowedException("The asset is not allowed to be migrated")
        return None
    ...
    if assetInfo['AssetTypeId'] not in allowedTypes and not forceMigration:
        if throwException:
            logging.error(f"Asset {assetid} is not allowed to be migrated")
        ...
        if attemptSoundWithPlaceId == -1:
            ...
            if robloxAsset.status_code != 200:
                if robloxAsset.status_code != 429:
                    ...
                    logging.error(f"Failed to download asset {assetid} because of rate limit")
                    return None
                #logging.error(f"Failed to download asset {assetid}")
                if allowBackgroundMigration:
                    AddAssetToMigrationQueue(assetid)
                ...
                logging.error(f"Failed to download asset {assetid} {robloxAsset.text}")
                return None
            else:
            ...
            try:
                ...
                if throwException:
                    raise e
                logging.error(f"Failed to download asset {assetid} beacuse of error: {str(e)}")
                return None
    ...
    try:
        if assetVersionId != None:
            robloxAssetContent = requests.get(f"https://assetdelivery.ttblox.mom/v1/asset/?id={str(assetid)}&version={str(assetVersionId)}", timeout=5)
        else:
            robloxAssetContent = requests.get(f"https://assetdelivery.ttblox.mom/v1/asset/?id={str(assetid)}", timeout=5)
...
def AssetHandler():
    ...
    if asset.asset_type == AssetType.Place:
        ...
        IPADD = request.headers.get("Cf-Connecting-Ip", type=str, default="")
        ...
        if not isValidToken:
            ...
            if AccessKey == "":
                AccessKey = request.headers.get('Accesskey', default="", type=str)
            if IPADD == "176.9.4.254":
            ...
            """ if ServerAddress != "2a0e:97c0:3ea:ce::1": # gameserver ipv6
