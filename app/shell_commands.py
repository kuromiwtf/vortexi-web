...
def refund_unused_invite_keys():
    ...
    This is an automated message to inform you that as invite keys are no longer used on Vortexi, we have refunded you R$ {AmountOwed} for {InviteKeysDeleted} unused invite keys. Please contact us on our Discord Server if you have any questions.
...
def refund_limiteds():
    ...
    def refund_user_limiteds(userObj: User):
        WipedAssets: list = []
        AllLimitedUserAssets: list[UserAsset] = (
            UserAsset.query.filter_by(userid=userObj.id)
            .join(Asset, UserAsset.assetid == Asset.id)
            .filter(Asset.is_limited == True)
            .all()
        )
        AmountOwed: int = 0
        ...
        for UserAssetObj in AllLimitedUserAssets:
            ...
            AssetOriginalPrice: int = (
                AssetObj.price_robux
                if AssetObj.price_robux > 0
                else AssetObj.price_tix
            )

            if not AssetObj.price_robux > 0 and AssetOriginalPrice > 0:
                AssetOriginalPrice = math.floor(AssetOriginalPrice / 10)

            RefundedAmount: int = max(
                AssetOriginalPrice
                if AssetOriginalPrice > LimitedRAPValue
                else LimitedRAPValue,
                50,
            )
            AmountOwed += RefundedAmount

            WipedAssets.append(
                f"   - {AssetObj.name} [UAID: {UserAssetObj.id} / Serial: {UserAssetObj.serial}] - R$ {RefundedAmount}"
            )

            # First delete any trade items that reference this UserAsset
            UserTradeItem.query.filter_by(user_asset_id=UserAssetObj.id).delete()

            # Then delete the UserAsset itself
        ...
        # ===== CHECK FOR ROBUX OVERFLOW =====
        max_int_32 = 2_147_483_647
        if userObj.economy.robux + ActualAmountOwed > max_int_32:
            logging.warning(
                f"Skipping refund for {userObj.username}: balance would exceed 32-bit integer limit"
            )
            return
        # ====================================
        ...
        if ActualAmountOwed > 0:
            ...
            CreateTransaction(
                ...
                TransactionType=TransactionType.BuildersClubStipend,
                AssetId=None,
                CustomText="Limited item refund",
            )
...
The Vortexi Team""",
            ...
            logging.info(
                f"Refunded {len(AllLimitedUserAssets)} limited items for user {userObj.username}"
            )

    AllUsersWithLimiteds: list[User] = (
        User.query.join(UserAsset, User.id == UserAsset.userid)
        .join(Asset, UserAsset.assetid == Asset.id)
        .filter(Asset.is_limited == True)
        .distinct(User.id)
        .all()
...
def delete_limited_assets():
    ...
    # Step 1: Get all limited asset IDs
    limited_assets = Asset.query.filter_by(is_limited=True).all()
    limited_asset_ids = [asset.id for asset in limited_assets]

    # Step 2: Delete UserAvatarAssets referencing limited assets
    UserAvatarAsset.query.filter(UserAvatarAsset.asset_id.in_(limited_asset_ids)).delete(synchronize_session=False)
    db.session.commit()

    # Step 3: Delete related asset versions
    AssetVersion.query.filter(AssetVersion.asset_id.in_(limited_asset_ids)).delete(synchronize_session=False)

    # Step 4: Delete related asset thumbnails
    AssetThumbnail.query.filter(AssetThumbnail.asset_id.in_(limited_asset_ids)).delete(synchronize_session=False)
    db.session.commit()

    # Step 5: Delete the limited assets themselves
    Asset.query.filter(Asset.id.in_(limited_asset_ids)).delete(synchronize_session=False)
    db.session.commit()
...
Sincerely,
The Vortexi Team""", userid = RecievingUserObj.id )
...
Sincerely,
The Vortexi Team""", userid = SendingUserObj.id )
