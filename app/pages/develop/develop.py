...
def ManageUniversePage( universeid : int ):
    ...
    if request.method == "GET":
        ...
        try:
            AssetYear : PlaceYear = PlaceYear(request.form.get("place_year", default=2018, type=int))
        except ValueError:
            flash("Invalid year", "error")
            return redirect(f"/develop/universes/{universeid}/manage")
        if AssetYear not in [ PlaceYear.Sixteen, PlaceYear.Eighteen ]:
            flash("Invalid year", "error")
            return redirect(f"/develop/universes/{universeid}/manage")
        ...
        if UniverseObj.place_year != AssetYear:
            # we check active servers
            TotalServersActive: int = PlaceServer.query.join(
                Place, Place.placeid == PlaceServer.serverPlaceId
            ).join(
                Universe, Place.parent_universe_id == Universe.id
            ).filter(
            ...
            all_places = Place.query.filter_by(parent_universe_id=universeid).all()
            for place in all_places:
                place.placeyear = AssetYear
