...
def before_request():
    if not auth.isAuthenticated():
        return redirect("/login")
    UserObj = auth.GetCurrentUser()  # Fixed line
    UserAdminPermission = AdminPermissions.query.filter_by(userid=UserObj.id).first()
    if UserAdminPermission is None:
        return redirect("/home")
...
def fflags_import(groupid: int):
    ...
    try:
        ...
        db.session.rollback()
        flash(f"Error importing flags: {str(e)}", "danger")
        return redirect(f"/admin/fflags/{str(groupid)}")

@AdminRoute.route('/fflags/create', methods=['GET', 'POST'])
def fflags_create():
    AdminPermissionRequired('ManageFFlags')
    ...
    if request.method == 'POST':
        ...
        gameserver_only = request.form.get('gameserver_only', 'false') == 'true'
        category = request.form.get('category', '2019+')  

        if not name:
            flash("Group name is required", "danger")
            return redirect("/admin/fflags/create")
        ...
        if FflagGroup.query.filter_by(name=name).first() is not None:
            flash("A group with this name already exists", "danger")
            return redirect("/admin/fflags/create")

        try:
            if category == "2019+":
                if not name.startswith("application_"):
                    name = f"application_{name}"

            last_group = FflagGroup.query.order_by(FflagGroup.group_id.desc()).first()
            new_group_id = last_group.group_id + 1 if last_group else 1
            
            current_time = datetime.utcnow()
            new_group = FflagGroup(
            ...
            new_group.gameserver_only = gameserver_only
            new_group.updated_at = current_time
            
            db.session.add(new_group)
            db.session.commit()

            flash("FFlag group created successfully", "success")
            return redirect(f"/admin/fflags/{new_group.group_id}")
            
        except Exception as e:
            db.session.rollback()
            flash(f"Error creating FFlag group: {str(e)}", "danger")
            return redirect("/admin/fflags/create")

    return render_template('admin/fflagsettings/create.html')
...
def asset_copier_post():
    ...
    # Migrate asset
    NewAsset : Asset = migrateAsset(int(AssetID), forceMigration=False, allowedTypes=[2, 8, 11, 12, 17, 18, 19, 41, 42, 43, 44, 45, 46, 47], creatorId=49, keepRobloxId=False, migrateInfo=True)
    if NewAsset is None:
...
def ManageUserAdminPermissionsSetPermission(userid: int):
    ...
    # Check for permissions data in the request
    if "permissions" not in UpdatedPermissionData:
        return jsonify({
            "success": False,
            "reason": "Bad Data: No permissions provided."
...
def BanUser(userid: int):
    ...
    LastestUserBanObj = UserBan.query.filter_by(userid=userObj.id, acknowledged=False).order_by(UserBan.id.desc()).first()
...
def BanUserPost(userid: int):
    ...
    BanTypeInput = request.form.get(key='ban_type', default=None, type=int)
    BanReason = request.form.get(key='ban_reason', default=None, type=str)
    ModeratorNote = request.form.get(key='ban_notes', default=None, type=str)

    if BanTypeInput is None or BanTypeInput not in [0, 1, 2, 3, 4, 5, 6] or BanReason is None or ModeratorNote is None:
        flash("Invalid request", "danger")
        return redirect(f"/admin/manage-users/{str(userid)}/ban-user")

    BanTypeInput = BanType(BanTypeInput)
    ...
    PlaceserverPlayerObj = PlaceServerPlayer.query.filter_by(userid=userObj.id).first()
    if PlaceserverPlayerObj is not None:
        CurrentPlaceServerObj = PlaceServer.query.filter_by(serveruuid=PlaceserverPlayerObj.serveruuid).first()
...
def ViewUserModeratorNotes( userid : int ):
    ...
    if PageNumber < 1:
        PageNumber = 1
    Robux : list[ModeratorNote] = ModeratorNote.query.filter_by( user_id = userObj.id ).order_by(ModeratorNote.created_at.desc()).paginate(page=PageNumber, per_page=15, error_out=False)

    def GetUserName( TargetUserId : int ) -> str:
        return User.query.filter_by( id = TargetUserId ).first().username

    return render_template(
        "admin/usermanage/currency.html",
        userObj = userObj,
        Robux = Robux,
...
def CreateGiftcardPost():
    ...
    if GiftcardTypeInput in [GiftcardType.RobuxCurrency, GiftcardType.TixCurrency]:
        AuthenticatedUser : User = auth.GetCurrentUser()
        if GiftcardValue > 100000000 and AuthenticatedUser.id not in (1, 49):
            flash("Giftcard value must be at most 100000000 (why do u want that much wtf??)", "error")
            return redirect("/admin/create-giftcard")
    ...
    if redis_controller.get(f"GiftcardCooldown_{AuthenticatedUser.id}") is not None:
        flash("You are on cooldown retard", "error")
        return redirect("/admin/create-giftcard")
...
def UpdateAssetFilePost():
    ...
    if AssetObj is None:
        flash("Invalid asset", "error")
        return redirect("/admin/update-asset-file")
    if AssetObj.creator_id not in [1,49] or AssetObj.creator_type != 0:
        flash("Asset is not owned by Vortexi", "error")
        return redirect("/admin/update-asset-file")
    if AssetObj.asset_type not in [AssetType(1), AssetType(2), AssetType(3), AssetType(4), AssetType(8), AssetType(11), AssetType(12), AssetType(17), AssetType(18), AssetType(19), AssetType(24), AssetType(27), AssetType(28), AssetType(29), AssetType(30), AssetType(31), AssetType(32), AssetType(41), AssetType(42), AssetType(43), AssetType(44), AssetType(45), AssetType(46), AssetType(47), AssetType(57), AssetType(58)]:
        if AuthenticatedUser.id not in (1, 49):
...
def LotteryPost():
    ...
    for UserAssetObj in EligibleLimiteds:
        ...
        CreateSystemMessage(
            ...
            f"""You have been randomly selected to be given a free limited by Vortexi.
...
""",
        ...
        CreateSystemMessage(
            ...
            f""""Your limited {UserAssetObj.asset.name} ( {UserAssetObj.asset.id} ) [ UAID: {UserAssetObj.id} / Serial: { UserAssetObj.serial if UserAssetObj.serial is not None else 'None'} ] has been removed from your account and was given to another ca User
